...
for(let i = 0; i < dikdortgenOlanlar.length/2 ;i++){
    if(dikdortgenOlanlar[2*i +1]!= undefined){ 
        toplamDikdörtgenAlanı += hesaplama.dikdörtgenAlani(dikdortgenOlanlar[2*i],dikdortgenOlanlar[2*i+1]);
...
toplamDaireAlanı += hesaplama.daire(daireOlanlar[k]);
}
...
for(let l = 0; l < kupHacimleri.length; l++){
    if(kupHacimleri[l] > 0){
    toplamKüplerinHacmi += hesaplama.Küp(kupHacimleri[l]);   
}
}
